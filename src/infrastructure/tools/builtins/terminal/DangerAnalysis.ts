...
import type { DangerLevel } from "@/application/tools/Types";
...
const PATTERNS: DangerousPattern[] = [
  { pattern: /\b(?:rm\s+(?:-[a-z]*r[a-z]*|--recursive)\b|remove-item\b[^\r\n;|]*-recurse\b|rmdir\s+\/s\b|del\b[^\r\n;|]*\/s\b)/i, level: "destructive", message: "This command can recursively delete files.", reasonCode: "destructive-delete" },
  ...
  { pattern: /\b(?:taskkill|killall|pkill)\b/i, level: "dangerous", message: "This command can terminate processes outside the active workspace.", reasonCode: "process-termination" },
...
export async function analyzeDangerLevel(command: string, context: TerminalAnalysisContext): Promise<TerminalDangerAnalysis> {
  ...
  const analysisCommand = stripNeutralStderrRedirection(command);
  if (isKnownSafeDiagnostic(analysisCommand, shellFamily)) {
    return {
      level: "safe",
      reasonCode: "allowlisted-read-only",
      normalizedCommand: analysisCommand.trim(),
      workspaceContained: true,
      shellFamily,
    };
  }
  const matched = PATTERNS.find((item) => item.pattern.test(analysisCommand));
  const likelyWorkspaceContained = isLikelyWorkspaceContained(
    analysisCommand,
    matched?.reasonCode,
  ...
  if (shellFamily === "unknown") {
    return result(
      matched?.level ?? "caution",
      matched?.message ?? "The resolved shell is not supported by the read-only allowlist.",
      matched?.reasonCode ?? "unknown-shell",
      shellFamily,
      undefined,
      likelyWorkspaceContained,
    );
  }

  const parsed = tokenize(analysisCommand, shellFamily);
  if (!parsed) {
    return result(
      matched?.level ?? classifyUnsupportedSyntax(analysisCommand),
      matched?.message ?? UNKNOWN_MESSAGE,
      matched?.reasonCode ?? "unsupported-syntax",
      shellFamily,
      undefined,
      likelyWorkspaceContained,
    );
...
function isKnownSafeDiagnostic(command: string, shellFamily: ShellFamily): boolean {
  const trimmed = command.trim();
  return shellFamily === "cmd" && /^(?:cd|echo\s+%cd%)$/i.test(trimmed);
}
...
function isLikelyWorkspaceContained(
  command: string,
  reasonCode?: string,
  workspaceRoot?: string,
  shellFamily: ShellFamily = "unknown",
): boolean {
  ...
  if (
    (reasonCode === "delete" || reasonCode === "destructive-delete") &&
    /[*?[\]{}]/.test(command)
  ) {
    return false;
  }
  if (hasAbsolutePathOutsideWorkspace(command, workspaceRoot, shellFamily)) {
    return false;
  }
  return !(
    /(?:^|[\s"'\\/])\.\.(?:$|[\s"'\\/])/.test(command) ||
...
function hasAbsolutePathOutsideWorkspace(
  command: string,
  workspaceRoot: string | undefined,
  shellFamily: ShellFamily,
): boolean {
  const candidates = new Set<string>();
  for (const match of command.matchAll(/["']((?:[a-zA-Z]:[\\/]|\\\\)[^"']+)["']/g)) {
  ...
  return [...candidates].some((candidate) => !isLexicallyInsideWorkspace(candidate, workspaceRoot));
}

function isLexicallyInsideWorkspace(candidate: string, workspaceRoot: string): boolean {
  const pathApi = path.win32.isAbsolute(candidate) || candidate.startsWith("\\\\")
    ? path.win32
    : path.posix;
  if (!pathApi.isAbsolute(candidate) || !pathApi.isAbsolute(workspaceRoot)) {
    return false;
  }
  const relative = pathApi.relative(pathApi.resolve(workspaceRoot), pathApi.resolve(candidate));
  return relative === "" ||
