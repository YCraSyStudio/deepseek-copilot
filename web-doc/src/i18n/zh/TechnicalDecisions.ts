  ...
  sections: [
    ...
    },
    {
      ...
      items: [
        ...
        "在 VS Code 中，每个命令都会通过 Shell Integration 在专用集成终端中可见执行；随后终端关闭，但会保留结构化有界输出、取消、超时和退出状态。系统拒绝分离式启动器，并禁用代理终端中的 .NET 构建服务器复用；非 VS Code 宿主继续使用 headless 执行器作为兼容回退。",
