...
export const userManual: PageContent = {
  ...
  sections: [
    ...
    },
    {
      title: "权限和工具状态",
      items: [
        "default 会确认每个工具；read-only 自动批准读取/列出/搜索，并保留需确认的修改工具；auto-approve 委托工作区内操作；full-access 在全局警告后允许不受限制地访问计算机。custom 可单独配置工具。",
        ...
        "同一轮内的工具调用按顺序执行，重复的同名同参数调用会被跳过。每到配置的轮次检查点，auto-approve 和 full-access 会让 DeepSeek 重新判断是继续使用工具、请求指示还是停止；其他模式则询问用户是否再批准一批轮次。",
